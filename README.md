...
ToDoWell 由 Python 3.8 + tkinter 的 1.5.0 版本完全用 C++17 与 Direct2D 重写（当前版本 2.5.9）。窗口可设置默认显示位置（默认右下角），支持项目分类管理、待办任务增删改、历史代办任务记录、中文输入法候选框跟随、多套归位/关闭动画、开机自启与 JSON 持久化。无任何第三方运行时依赖。
